...
base = {'A':'OP(=O)(O)OCC1OC(N3C=NC2=C(N)N=CN=C23)CC1',
        ...
        'E':'OP(=O)(O)OCC1OC(N2C(=O)N=C(N)C(C)=C2)CC1',
        'I':'OP(=O)(O)OCC1OC(N3C=NC2=C(NC)N=CN=C23)CC1'}
#modifications
#E, 5mC, OP(=O)(O)OCC1OC(N2C(=O)N=C(N)C(C)=C2)CC1
#I, 6mA, OP(=O)(O)OCC1OC(N3C=NC2=C(NC)N=CN=C23)CC1
dna_smiles = kmer_chemistry.get_kmer_smiles(5, base)
kmer_chemistry.plot_molecule(dna_smiles.get('AICET')[0])
#DNA SMILES strings

f = open('./nanopolish_models/original/r9_250bps.nucleotide.5mer.template.model')
...
A, X = kmer_chemistry.get_AX_matrix(smiles, ['C', 'N', 'O', 'P'], 111)
...
nn_model.save_tuning_history('dna_r9_native', history)
...
nn_model.save_model('dna_native_r9', model)
#fit model
